...
parse_metadata_AML.miRNA.2016 <- function() {
    ...
    assay <- "miRNA"
    tissue <- "PBMC"
    sample_sheet <- read.csv(here::here(work_dir, "data-raw/2016miRNA_new_name_key.csv")) |>
        dplyr::mutate(
            cohort = cohort,
            assay = assay,
            tissue = tissue,
            ...
            timepoint = stringr::str_replace(newname, "^\\d{4}-(.?)\\.fq", "\\1"),
            timepoint = case_when(timepoint != "L" ~ as.character(paste0("T", as.numeric(timepoint) - 1)), TRUE ~  timepoint),
...
parse_metadata_AML.miRNA.2018 <- function() {
    ...
    sample_sheet <- read.csv(here::here(work_dir, "data-raw/2018miRNA_new_name_key.csv")) |>
        dplyr::mutate(
            cohort = cohort,
            assay = assay,
            tissue = tissue,
...
parse_metadata_AML.miRNA.2021.RxGroup2_pt2 <- function() {
    ...
    sample_sheet <- list.files(
        ...
        dplyr::mutate(
            ...
            assay = assay,
            fastq_1 = value,
            mouse_id = stringr::str_replace(value, ".*/(?:[^_]+_){1}(\\d{4}).*", "\\1"),
            timepoint = stringr::str_replace(value, ".*/(?:[^_]+_){1}[^_]*-([^_]*)_.*", "\\1"),
            batch = batch,
            tissue = tissue,
            tissue = dplyr::case_when(
                timepoint == "BM" ~ "BM",
...
parse_metadata_AML.miRNA.2022.RxGroup3 <- function() {
    ...
    sample_sheet <- readxl::read_excel("/labs/ykuo/Seq/220620_IGC-LZ-20205/sample summary_IGC-LZ-20205.xlsx") |>
        dplyr::mutate(
            library_id = paste0("COHP_", stringr::str_replace(Sample_ID, "(\\d*)_.*", "\\1")),
            cohort = cohort,
            assay = assay,
            mouse_id = stringr::str_replace(Sample_ID, "(?:[^_]+_){2}(\\d{4}).*", "\\1"),
            timepoint = toupper(stringr::str_replace(Sample_ID, "(?:[^_]+_){2}\\d{4}(.*)", "\\1")),
            timepoint = gsub("-2|_", "", timepoint),
            batch = batch,
            tissue = tissue,
            tissue = case_when(
                timepoint == "BM" ~ "BM",
