...
Source: [preprocessing/\_targets.qmd](https://github.com/cohmathonc/haemdata/blob/main/preprocessing/_targets.qmd)
...
The [{`targets`}](https://books.ropensci.org/targets/) package streamlines data workflows and promotes reproducibility. It skips redundant tasks and abstracts files as R objects to ensure efficient, painless, and trustworthy results. This pipeline is run as a [Target Markdown](https://books.ropensci.org/targets/markdown.html) document, where the pipeline is defined within this Quarto file. 
:::
...
The code to collate it has been through multiple iterations, is complex and ugly and undoubtedly some mistakes will have made it through. Errors or omissions can be reported on [GitHub](https://github.com/cohmathonc/haemdata/issues).
...
- adds the metadata for the `senescence.mRNA.2022` cohort (3 young, 3 old mice)
...
For **hsa** metadata, `make_metadata_hsa()` row binds all available metadata and where possible, 
extracts `study_title`, `tissue`, `patient_id`, `sex`, `diagnosis_year`, `clinical_treatment`, `date_of_diagnosis`, 
`age_at_diagnosis`, `sample_date`, `timepoint`, `genotype`, `treatment`, `dob`, and assigns a `cohort` and (sequencing) `batch`.

Many fields are not available across all datasets. For instance, `clinical_treatment` is only available for COH patients; 
MDS & CBF-AML patients have minimal metadata as no per-sample information is available.
...
The workflows for all assays begin with raw sequencing reads for bulk miRNA and mRNA, or h5ad files for 10X Cellplex libraries.
...
-   2022 Senecence mice (3 young, 3 old)
...
list(
    ...
    tar_target(sample_sheet_mrna_old, make_rnaseq_sample_sheet("^senescence.mRNA.2022$"))
)        
```

Spaces are not permitted in filenames, so for these cohorts, we need to cache and symlink fastqs:
...
-   Human sample metadata is yet to be standardized between experiments, so they remain separate for now.
...
Merge the late-coming senescent mice cohort and mir142 KO CML samples with all other mice into a single SummarisedExperiment.
...
-   Others on [GitHub](https://github.com/cohmathonc/haemdata/issues/7)
