...
The current implementation of `update_metadata_mmu()` changes `sample_id` prefix to MHO from PSON, 
simplifies `fastq_1` & `fastq_2` paths, add @yufu1120 sample metadata and adds CML miR-142 KO scRNAseq (LMPP & T cells) metadata.
...
list(
    ...
    #     annotate_se(
    #         get_rnaseq_se(mmu_mrna_mir142_qc),
    #         metadata_mmu_prepub,
    #         mmu_mrna_mir142_qc
    # ))
)
...
list(
    ...
    tar_target(mmu_mirna_all_mice_mirna_qc, rbind(mmu_mirna_pretrimmed_qc, mmu_mirna_trimmed_qc))
)
```
...
The [`seurat_import_cellplex()`](http://cgt.coh.org/haemdata/reference/seurat_import_cellplex.html) function builds Seurat objects and adds cell metadata columns indicating the percent expression of mitochondrial, ribosomal, haemoglobin, platelet, _Xist_ & chrY genes, and the transgene _MYH11_.
...
```{targets link_fastqs_10x_mir142, eval=TRUE}
...
The [`seurat_import_cellranger()`](http://cgt.coh.org/haemdata/reference/seurat_import_cellranger.html) function builds Seurat objects and adds cell metadata columns indicating the percent expression of mitochondrial, ribosomal, haemoglobin, platelet, _Xist_ & chrY genes, and the transgene _MYH11_.
...
list( 
    ...
    tar_target(
        ...
        publish_mirna_qc(mmu_mirna_all_mice_mirna_qc),
        deployment = "main"
    )
)
```
